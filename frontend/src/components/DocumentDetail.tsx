...
export const DocumentDetail: React.FC = () => {
	...
	// Hooks must always be called – pass fallback id (empty string) but rely on enabled flag in hook
		...
		return (
			<div className="bg-red-50 border border-red-200 rounded-md p-4">
				<div className="flex">
					...
					<div className="ml-3">
						<h3 className="text-sm font-medium text-red-800">Document id missing</h3>
						<div className="mt-2 text-sm text-red-700">
							<p>Cannot load document without an id.</p>
						</div>
					</div>
				</div>
			</div>
		...
		if (id) {
			...
			analyzeMutation.mutate(id, {
				onSuccess: () => {
					...
					const pollingToast = toast.info('Analyzing... this may take up to a minute.', {
						durationMs: 0,
					});
					setIsPolling(true);
			...
			<div className="bg-red-50 border border-red-200 rounded-md p-4">
				<div className="flex">
					...
					<div className="ml-3">
						...
						<div className="mt-2 text-sm text-red-700">
							<p>
								Could not find the requested document. It may have been removed or the link is
								invalid.
							</p>
						</div>
					</div>
				...
				{document.fullTextUrl && (
					<div className="mb-4">
						...
						>
							...
							{document.fullTextUrl.includes('congress.gov')
					...
					<div className="mb-4">
						<div className="w-full bg-gray-200 rounded-full h-3 mb-3">
							<div className="flex h-3 rounded-full overflow-hidden">
								{/* Progress bar removed to satisfy lint rule against inline styles. */}
								<div className="text-xs text-gray-600 space-x-2">
									<span>Democrats: {democraticPercentage.toFixed(1)}%</span>
									<span>Republicans: {republicanPercentage.toFixed(1)}%</span>
									{100 - democraticPercentage - republicanPercentage > 0 && (
										<span>
											Other: {(100 - democraticPercentage - republicanPercentage).toFixed(1)}%
										</span>
									)}
								</div>
							</div>
						...
						<div className="flex justify-between text-sm text-gray-600">
							<span>
								Democrats: {document.partyBreakdown.democratic} ({democraticPercentage.toFixed(1)}%)
							</span>
							<span>
								Republicans: {document.partyBreakdown.republican} ({republicanPercentage.toFixed(1)}
								%)
							</span>
						...
						{document.actions.slice(0, 10).map((action) => (
							<div key={action.id} className="flex">
								...
								<div className="flex-1 ml-4">
									...
									{action.chamber && (
										<p className="text-xs text-gray-500 mt-1">Chamber: {action.chamber}</p>
									)}
								</div>
							</div>
						))}
