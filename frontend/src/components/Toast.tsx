...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const enqueue = useCallback((toast: Omit<Toast, 'id'>) => {
    const id = (globalThis as any).crypto?.randomUUID ? (globalThis as any).crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
