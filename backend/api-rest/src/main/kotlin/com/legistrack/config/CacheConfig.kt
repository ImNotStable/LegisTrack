...
/**
 * Redis cache configuration and structured key generator (migrated from root module so the
 * aggregator no longer contains production sources).
 */
class StructuredKeyGenerator : KeyGenerator {
    override fun generate(target: Any, method: java.lang.reflect.Method, vararg params: Any?): Any {
        ...
        val serializedParams = params.joinToString(separator = "|") { param ->
            when (param) {
                null -> "null"
...
open class CacheConfig {
    ...
    @Bean
    open fun keyGenerator(): KeyGenerator = StructuredKeyGenerator()
}
