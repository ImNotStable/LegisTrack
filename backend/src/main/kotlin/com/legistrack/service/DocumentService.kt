...
class DocumentService(
    ...
    fun getAllDocuments(pageable: Pageable): Page<DocumentSummaryDto> {
        return documentRepository.findAllWithValidAnalyses(pageable)
            .map { document -> document.toSummaryDto() }
    }
    ...
    fun getDocumentById(id: Long): DocumentDetailDto? {
        return documentRepository.findByIdWithDetails(id)?.toDetailDto()
    }
    ...
    private fun Document.toSummaryDto(): DocumentSummaryDto {
        ...
        val partyBreakdown = sponsors.calculatePartyBreakdown()
        ...
        return DocumentSummaryDto(
            ...
            status = status,
            industryTags = validAnalysis?.industryTags?.toList().orEmpty(),
    ...
    /**
     * Converts a Document entity to a detailed DTO.
     */
    ...
    private fun List<DocumentSponsor>.calculatePartyBreakdown(): PartyBreakdownDto {
        val partyGroups = groupBy { sponsor ->
