...
class ExternalApiServicesTest {
    ...
    fun setup() {
        ...
        govInfoApiService = GovInfoApiService(
            webClient = mockWebClient,
            apiKey = "test-key", 
    ...
    @Test
    fun `CongressApiService can fetch recent bills`() = runBlocking {
        // Arrange
        val mockResponse = CongressBillsResponse(
            bills = listOf(
                CongressBill(
                    congress = 118,
                    number = "1234",
    ...
    @Test
    fun `CongressApiService can fetch bill details`() = runBlocking {
        // Arrange
    ...
    @Test
    fun `GovInfoApiService can fetch collections`() = runBlocking {
        // Arrange
        val mockResponse = GovInfoCollectionsResponse(
            count = 1,
            collections = listOf(
                GovInfoCollection(
    ...
    @Test
    fun `GovInfoApiService can search documents`() = runBlocking {
        // Arrange
        val mockResponse = GovInfoSearchResponse(
            count = 1,
            packages = listOf(
                GovInfoSearchResult(
    ...
    @Test
    fun `services handle errors gracefully`() = runBlocking {
    ...
    fun `DTOs are properly structured`() {
        ...
        val govInfoPackage = GovInfoPackage(
